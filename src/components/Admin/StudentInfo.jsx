        ...
        </tr>
        {
            ...
            (item,index) => (
                <tr style={{border:"1px solid black", padding:5}} key={item._id}>
                    ...
                    <td style={{cursor:'pointer', border: '1px solid #000', padding: '8px', }}>{item.studentName}</td>
                    <td style={{border: '1px solid #000', padding: '8px', }}><button>Edit</button>   
                    <button style={{backgroundColor:'red'}} onClick={() => handleDelete(JSON.stringify(item._id))} >Delete</button>
                    </td>
                </tr>
            ))
        }
