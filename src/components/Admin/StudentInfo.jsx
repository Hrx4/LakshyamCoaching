...
const StudentInfo = ({ studentList, setStudentList }) => {
  ...
  return (
    ...
    item.studentName, item.studentClass,item.studentBoard, item.studentCourse , item.studentSubjects , item.studentEmail , item.studentPhone , item.studentAddress , 
    item.studentPaymentType , 
    item.studentDob , item.studentPhoto , item.guardianName , item.guardianPhone , item.guardianEmail , item.guardianAddress
                )} 
                >Edit</button>
                <button
          ...
          <Box sx={{position: 'absolute',
            ...
            <FormControl style={{width:'60%', backgroundColor:'white'}} className="student__field">
              ...
              :null
                ...
                <input type="file" name="photo" accept="image/*" style={{width:'40%', marginRight:10}} onChange={uploadFiles} />
                </div>
              </div>
              <div className="form-part" style={{width:"100%"}}>
                  <h2 className="guardianHeading" style={{marginLeft:40}}>Guardian Details</h2>
